...
bool MemberRpts::getFormerMbrInfo(MemberRcd& rcd, MemberInfo& memberInfo) {
...
StatusRcd* statusRcd = statusTbl.find(statusID);
  ...
  EntityRcd* mbrRcd = entityTbl.find(rcd.mbrEntityID);   if (!mbrRcd) return false;
...
bool MemberRpts::getMemberInfo(MemberRcd& rcd, MemberInfo& memberInfo) {
int        statusID  = rcd.statusID;
StatusRcd* statusRcd = statusTbl.find(statusID);
...
void MemberRpts::display (MemberRcd& rcd) {
bool             comparison      = dspType == CmpDspType;

EntityRcd*       mbrRcd          = entityTbl.find(rcd.mbrEntityID);
EntityRcd*       emplRcd         = entityTbl.find(rcd.emplEntityID);
EntityRcd*       iceRcd          = entityTbl.find(rcd.ice_EntityID);
AssgnPrefRcd*    assgnPrefRcd    = assgnPrefTbl.find(rcd.assgnPrefID);
LocationPrefRcd* locationPrefRcd = locationPrefTbl.find(rcd.locationPrefID);
...
bool MemberRpts::dspEntity(EntityRcd* ent, MemberRcd* rcd) {
AddressRcd*   addrRcd;
CityStateRcd* cityRcd;
  ...
  addrRcd = addressTbl.find(ent->addrID);
  ...
  cityRcd = cityStateTbl.find(ent->cityStID);
