...
void GoogleRpt::display(MemberRcd& rcd, bool workAddr) {
...
EntityRcd*       mbrRcd          = entityTbl.find(mbrID);      if (!mbrRcd) return;
EntityRcd*       emplRcd         = entityTbl.find(emplID);
AddressRcd*      addrRcd;

AssgnPrefRcd*    assgnPrefRcd    = assgnPrefTbl.find(assgnPrefID);
LocationPrefRcd* locationPrefRcd = locationPrefTbl.find(locPrefID);
StatusRcd*       statusRcd       = statusTbl.find(statusID);
String           s;
String           addrType        = workAddr ? _T("Work") : _T("Home");

EntityRcd* r     = workAddr ? emplRcd : mbrRcd;
...
bool GoogleRpt::addrAvail(EntityRcd* rcd, AddressRcd*& addrRcd) {

  if (!rcd) return false;

  addrRcd = addressTbl.find(rcd->addrID);

  return addrRcd && (!addrRcd->address1.isEmpty() || rcd->addrIsPO);
  }


void GoogleRpt::entityOut(EntityRcd* rcd, AddressRcd*& addrRcd) {
CityStateRcd* cityRcd = cityStateTbl.find(rcd->cityStID);
String        s;
