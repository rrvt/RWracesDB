...
void Everbridge::display(MemberRcd& rcd) {
EntityRcd*       mbrRcd            = entityTbl.find(rcd.mbrEntityID);      if (!mbrRcd) return;
...
void Everbridge::location(TCchar* title, EntityRcd* rcd) {
AddressRcd*   addrRcd = 0;
CityStateRcd* cityRcd = 0;
String        s;
String        t;

  if (rcd) {
    addrRcd = addressTbl.find(rcd->addrID);
    cityRcd = cityStateTbl.find(rcd->cityStID);
    }
